...
|---|---|---|---|
| **Creeds** | 3–5, pick 1/run | A run-long temperament that reshapes the verb's risk | Curated, never rarity-rolled. Include: one forgiving/crossover, one greed pole, one rhythm-changer, and **one WILD creed that rewrites the verb** (Tutti-class) |
...
| **Boons** | 10–16 | The draftable pool, in LANES named after the verb's dials | Every card names the dial it touches (the address rule). Each lane: ≥1 greed card, ≤1 insurance card |
...
| **Keystones** | 2–3 | Build-definers dropped by ELITES only, never normal drafts | **Spectacle-grade, never a stat.** Bill's bar: "keystones need to be way more fun than open kick window." If it doesn't change how the bar/verb LOOKS in play, it isn't a keystone |
| **Support** | 1 | The party-facing identity card | Keyed to the spec's core state (uptime IS the buff) |
| **Carries** | listed | Cross-spec cards that work VERBATIM on both specs | Verify each against the new verb — no "mostly works" |

Curios are the only cross-CLASS lane and never touch a verb. Spell/extra-button lanes are
...
3. **Control** — agency over the verb's randomness (rerolls, placement bias, pins).
...
- **Write the DIALS list first.** Enumerate what the verb lets a card touch (for Fermata:
...
- **When the verb changes, run the BROKE / FADED / DEAD / OPENED sweep** over the whole
...
- Wideners/forgiveness effects TAPER with power (help the cold player, not the hot one),
  and under edge-graded verbs they extend the SAFE side only — never move the payoff lip.
