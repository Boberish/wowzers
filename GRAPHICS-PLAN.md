...
different agents — this is the frozen seam between them so the UI team builds while the combat
team keeps implementing.** The current live surface is `game/ui/answer_channel.gd` +
`game/ui/bands/duelist_band.gd`; C6 re-homes them into the painted theater/instruments split
above without changing this data. **Draw only what these two surfaces carry — never invent,
predict, or re-grade** (LAW 1). Verified against the code 2026-07-13.

**A · THE ANSWER CHANNEL — comet vocabulary (ONE BAR: everything answerable, one channel).**

| kind | shape · color | word | answered by |
|---|---|---|---|
| `auto` | gold diamond (LIGHT) | DODGE | dodge any grade, or parry |
| `heavy` | amber hexagon | PARRY | parry, or a BULLSEYE dodge |
| `buster` (telegraph, aimed at you, no beats) | crush-red spiked octagon | PARRY | parry (±window) or bullseye dodge |
| `global` (telegraph aoe beat) | crimson spiked octagon, gold spikes | DODGE | dodge any grade — **never parry** |
| `beat` (telegraph non-aoe, aimed at you) | crimson diamond | DODGE | dodge |
| `feint` | ANY real shape + **purple tint + breathing purple ring** | (lies) | HOLD — press = BAITED |
| `eat` | grey diamond + skull/✗ | EAT | brace (no press) |
| `flurry` | cyan bead cluster | WEAVE | dodge-only mode |

**B · OBS (per frame — `CombatCore.observe` → `duelist_kit.observe`): the STATE to bind.**
`stream.bars[]` = `{id, kind, purple, eta` (sec to gate)`, late, peeled, victim, answered,
flurry_i, flurry_n}` · `stream` = `{tempo, flurry, paused, cadence}` · `telegraph` =
`{tick, remaining, targets_me, defensible/heal/empower/interruptible/feint, strikes[]:{remaining,
aoe, mine, feint, resolved, answered}}` · `flow` (0..1 aggro %) · `flow_lock` (the 30% line) ·
`wind`/`wind_max` · `combo`/`combo_max` · `engarde_live`/`engarde_ready` · `fumbling` ·
`dodge_ready`/`parry_ready` · **gate geometry** `win_bullseye/win_perfect/win_good/win_graze`
(fractions of the claim window, **SYMMETRIC around gate-touch**) + `parry_window` (= parry_land).
The UI reads these — it **never bakes the numbers**.

**C · EVENTS (the MOMENTS — drained each frame, never checksummed).**
`duel_dodge`/`duel_parry {}` = the PRESS echo (gate kicks white + rune kick, the frame you
press) · `duel_answer {kind, grade, size, off_ms, id}` = the graded verdict — **`id ≥ 0` = a
stream bar, `id < 0` = a telegraph comet**; resolve/kill THAT comet with a burst at its frozen
last-drawn pixel + the ±ms readout · `duel_bar_missed {id, size}` = an unpressed damage bar
crossed the line → red ✗ husk, flows to the bar's end · `duel_fumble {}` = winded/dry press
(crimson gate tick) · `duel_counter {}`/`duel_riposte {}` = parry counter (+◆)/flurry riposte ·
`duel_weave_blown {}`/`duel_parry_sealed {}` · `duel_eat {}` = brace · `duel_engarde {}`/
`duel_engarde_break {}` · `duel_dump {amt}` · `stream_shatter {}` = full body-death shatter ·
...
shared judge are HIDDEN for the tank. **ONE CLAIM** — a press claims the NEAREST comet (stream OR
telegraph), judged instantly on the ladder; the gate draws only the true pressable window (full
on approach, ~1 tick past the line). **THE GUARD** — quiet windows open around answerable
telegraph impacts; committed comets inside shatter (`stream_guard_shatter`), the boss "rears up."
**MISS AFTERLIFE** — an unpressed comet crosses the line, turns red ✗, keeps flowing to the bar's
end. **PEEL** — a bar with `peeled=true` + a `victim` name draws a crimson hunt-chevron + "→
VICTIM"; the tank still answers it. **PRESS ECHO is instant**; the graded verdict + burst (fading
vertical line + expanding circle, the Twinfang idiom) anchor at the comet's frozen pixel.

**F · DASHBOARD INSTRUMENTS (`y 750–1040`):** HP orb · Flow/Aggro orb (% + the 30% lock line) ·
...
**G · STABLE vs IN-FLIGHT:** A–F (vocabulary · events · obs fields · behaviors) are **LOCKED** —
safe to build against. Still TUNING (numbers only, no shape/field change): guard window sizes
(`stream_guard_before/after`), per-Seal songbooks, grade fractions. Any change to A–F ships a
note in this section + the MASTER-PLAN log in the SAME commit.
