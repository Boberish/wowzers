...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P2 foundation** (C0 done · **C1 🔨 `3da278f`** · C2 next) | **`GRAPHICS-PLAN.md`** (SUNPRINT CEL + reaction-first verdict locked 07-12; §10 seam map) | LIVE `game/art_v2/art_v2.gd` (static `ArtV2`, three independent flags, default OFF; `--artv2=` parsed at `WorldShell._ready` head) · **seams now guarded in code (C1):** `Actor2D.make()` head (missing V2 asset ⇒ fall through; the post-purge duelist/alchemist/well→RiftmawRig2D fallthrough locked AS-IS by `sim/artv2_probe.gd` — C4 owns the fix) · `raid_hud._ready` backdrop via `ArtV2.make_scene()` (C2 replaces the lookup with SceneKit) · `_build_combat` v2dash guard (`make_dash` null until C6 registers a host + null-guards the render feed) | **Bill V1 approved:** bright, playful, detailed SUNPRINT CEL; UI is the game — dominant answer channel, immediate press/verdict, compact secondary abilities, fast cancelable actor payoff. Old renderer/HUD remains default. Mandatory bright-outdoor + darker-interior contrast proof. `stage2d/*`+backdrop = quiet; `raid_hud`/`duelist_band` = live tank surface, merge-main-per-slice. |
