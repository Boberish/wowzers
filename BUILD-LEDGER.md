...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **V1 style verdict** (C0 recon 🔨 done 07-12) | **`GRAPHICS-PLAN.md` §10** (seam map landed) | future `game/art_v2/` namespace · **exact seams (C0):** `Actor2D.make()` (`actor_2d.gd:58`, + fix the post-purge duelist/alchemist/well→RiftmawRig2D fallthrough) · the ONE `StageBackdrop` at `raid_hud._ready:276` · `_build_combat:2583`+`ClassBand.for_hud` · selector = static `ArtV2` + `--artv2=` in `WorldShell.drive_autostart` | **P0 docs + C0 recon done; no code yet.** Old renderer/HUD remains default; no code/art before Bill V1. Mandatory interior+outdoor contrast proof. `stage2d/*`+backdrop = quiet, low collision; `raid_hud`/`duelist_band` = live tank surface, merge-main-per-slice. |
