...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P6 BUILT: C7 🔨 MERGED 2026-07-14 (branch `artv2-c7`, Bill-directed) — VFX runtime + 13/13 transplant; ⚠ Bill's live verdict. P5 FULL ANATOMY REOPENED IN PARALLEL: I3-C dream dashboard active (Codex), then the Claude layout-copy packet.** P5 C6B `28e9b15`; I4 `2baf3fe`; I3-B `801d713`; C6A `2b407c4`. | **`GRAPHICS-PLAN.md` §2.3/§2.4 · P5/P6 · §5·C7 (as-built) · I3-C** | `sim/artv2_vfx_prep.gd` + `game/art_v2/vfx/*` (8 atlases + manifest + VfxBook/VfxPlayer/VfxPool) · `art_v2.gd` (+`vfx` token) · `raid_stage_2d.gd` (bindings + hitstop/lunge/smear) · `raid_hud.gd` (`_post` + washes + shield feed) · `ui/screen_post_fx.gd` NEW + `screen_post.gdshader` (+protect, default no-op) · `pose_rig_2d.flash_all` · `sim/artv2_vfx_tour.gd` NEW · `artv2_probe` (155) | Default OFF ⇒ byte-identical (ab-gate raid/duelist/twinfang). `art-Test` boots `dash,vfx`. **C7's old "held for final geometry" is RESOLVED by construction** — VFX ride live actor/channel geometry, so the I3-C layout-copy inherits them (re-run the vfx tour as the re-proof). Remaining P6 debt = visual-only (MASTER log). |
