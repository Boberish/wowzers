...
| `game/raid_hud.gd` (4496) | Every class gauge · aggro box · tank/dodge/drop UI · **Art V2 C6A dashboard host/layout routing** · **C7 `_post` washes + shield feed (landed 07-14, `ArtV2.vfx`-guarded)** · queued Meter L4/L5 | C6A stays additive and `ArtV2.dash`-guarded; default-off builds the whole legacy HUD. C7's event-map wash lines are additive inside `_handle_event` — Seal-rework edits to the same branches must keep the `_post != null` guards. Merge current `main` before the slice and do not mix Seal/Meter work into it. |
...
| **CHARGED PARRY + WEAVE REWORK (the big-swing answer)** | 🔨 **MERGED `d91bb8d`** | **TANK §11** (Bill 2026-07-14) | `combat_core.gd` (`perform` new `defense_release` + flurry rhythm draw in `_stream_push` ×2 paths) · `class_kit.gd` (no-op release hook) · `data/duelist/*` (gather state machine + knobs + trainer) · `policies/duelist_policy.gd` · `duelist_band.gd` + `answer_channel.gd` (hold/release input + charge arc) · `raid_content.gd` (per-Seal weave phrases ONLY — CRUSH swings keyed off size, no boss edits) · `net_protocol` bump | CRUSH busters = press-and-HOLD ≥½ wind-up, RELEASE on impact (charged counter, ◆◆ full-gather; short/early = FLINCH); heavies dodge-legal ONLY mid-hold. Weave n 6 / gap 0.26 / seeded jig / authored `gaps` / riposte ×(n/4). ⚠ deliberate fight-checksum re-baseline (stream rng order). `charge_enabled` = playtest A/B. |
...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P5/P6 BUILT: C6C 🔨 BUILT+MERGED `ccc35e8` (2026-07-14, `dash-c6c`) — dream-dashboard runtime copy; C7 🔨 MERGED — VFX runtime + 13/13 transplant. ✅ Bill live verdict: “for now it's solid”; merge/consolidate. P7/default-on remains a separate explicit ship decision.** P5 C6B `28e9b15`; I4 `2baf3fe`; I3-B `801d713`; C6A `2b407c4`. | **`GRAPHICS-PLAN.md` §2.3/§2.4 · P5/P6 · §5·C6C/C7 · I3-C** | C6C: `dash_host_c6a.gd` responsive anatomy · `dash_party_row.gd` · `answer_channel.gd` · `duelist_gauge.gd` · `ability_rune.gd` · updated `artv2_dash_states.gd`/`artv2_probe.gd` · new deterministic `artv2_dash_tour.gd`. C7 remains the VFX runtime/stage/post stack and tours. | Default OFF preserved; no baked anchor/gameplay-truth change. Passed import, probe 198/198, UI smoke, 36 exact-size matrix captures, both C7 tours, native legacy strip, scratch missing-assets fallback, focused byte-identical A/B. ✅ Live hierarchy/VFX accepted for now; P7/default-on stays explicit. |
...
| Charged-parry ART PASS (§2.3.2 brief) — actor GATHER/RELEASE/FLINCH anims · boss BIG-ONE telegraph dressing · charge VFX cues · charge-bar dashboard home | 🟡 brief frozen (`7bcc06c`, consolidated to main `731f2a3`); build unclaimed | **GRAPHICS-PLAN §2.3.2** + TANK-PLAN §11 pointer | `art_v2/actors/duelist` poses/frames · boss rig telegraph dressing · `vfx` families (possible new I4-style sources — IMAGE STOP applies) · `answer_channel`/dash charge-bar seat | Presentation-only; the §11.1 CHARGE MODE interim (wash/plaque/bar/halo, `1141908`+C6C-merged) is LIVE and stays until this pass replaces it. New image generation requires Bill's explicit go. |
| Twinfang art pass v1 (juice → painted skin → flipbook FX) | 🔨 **SALVAGE PAID — C7 transplanted all 13 hunks 2026-07-14** (both fixes applied: `_post=null` in `_clear` · finisher wash player-gated via one-blade-seat law; everything behind `ArtV2.vfx`, default OFF) | `ART-PLAN.md` (frozen) → **GRAPHICS-PLAN §10.5** (the hunk table, now marked transplanted) | `raid_hud` post-fx trigger map · `ui/screen_post_fx.gd` · `stage2d/raid_stage_2d` hit-stop/lunge/smear · `pose_rig_2d.flash_all` | **`tempo-art` (`e4589a6`, worktree `../wow-tempo-art`) is FULLY ABSORBED — frozen for deletion.** No clash with the AnswerChannel (timing truth not under `_world`); plain strikes keep no hit-stop. |
