...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **V1 style verdict** | **`GRAPHICS-PLAN.md`** (2026-07-12) | future `game/art_v2/` namespace · independent actor/scene/dashboard selector · Scene Profile host · painted actor adapter · Duelist rig · dashboard host · flipbook runtime · visual tours; exact seams land via Claude C0 recon | **P0 docs only.** Old renderer/HUD remains default; no code/art before Bill V1. Mandatory interior+outdoor contrast proof. Claude owns C0–C8 engineering packets; Codex owns I0–I5 image/visual packets. All code in fresh `art-v2` worktree, merge main frequently. |
...
| Twinfang art pass v1 (juice → painted skin → flipbook FX) | 🔮 **SUPERSEDED** · Slice 1 candidate `e4589a6` | `ART-PLAN.md` (frozen) → **GRAPHICS-PLAN §2.4/C0/C7** | old branch touches `raid_hud` post-fx · `stage2d/raid_stage_2d`/`pose_rig_2d` · damage-number styles; unbuilt rigid skin files/assets are cancelled | **Do not merge branch wholesale.** Claude C0 audits every Slice-1 hunk against current main; C7 may transplant reusable juice. Rigid AtO-cel Twinfang skin/"Bill generates PNGs" assumptions are retired. |
