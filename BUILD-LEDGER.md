...
| Interrupt-by-ability pillar | 🔒 | WORLD §PILLARS #3 **+ `BOSS-PLAN §1½` (the kick contract, 07-10)** | `AbilityRes.interrupts` flag (class-side) + `AbilityRes.kick_window` slice + castbar lit-slice + verse-table sim diag (boss-side, BOSS-PLAN E8) | Lands class-by-class w/ reworks (Tempo first). **07-10 amendments (Bill): EVERY class but healers carries ONE kick on its dump (2/1/0 retired — 3 kickers/warband) · window = small ABSOLUTE slice at cast end (~0.6 s, SealTune knob) · missed kick = the raid's costliest single mistake (blast → EMPOWER → boss-heal ladder).** Open Q: which Tempo ability carries — **proposal 🟡 at Bill (TEMPO §17.10, 07-10): Evis = standard carrier (dump tax), Coup = premium kick.** |
...
| Well — module gauges + AI spell-policy | ⏳ | MENDER | `well_gauge.gd`, `well_policy.gd` | Deck built; **AI now casts SKIN + pilots the Vigil hold (bank/release) + Second Hand off-hand flash (`ed358aa`/`270b420`)** — byte-neutral undrafted. Still owed: AI Meditate/Boiling Over (⚠ note the pre-existing `boil` book-key vs `boilingOver` boon-id mismatch); the module GAUGE renders (Vigil tremble/hold clock, flume/frozen states — exposed in observe, WSLg render = WELL-DRAW-BRIEF S5, deferred). |
...
| Curse cards (biting blessings) | 🔨 `a22c1ec` (welded-boons/DECK ⏳) | TEETH §CURSE | welded-downside boons + poisoned-ability flag | **SHIPPED via DESCENT §I slice 4 as THE JAILBREAK** (two-deal node + curse system: ECONOMY/HP/TIMING bites, cap 2, DEPRECATE/Cooling exits, no-run-long-timing HARD RULE). Owed: the welded-downside DRAFT boons (② door) + the poisoned-ability DECK tax (③, run-length — needs the `perform()` gate + spec-threading). |
...
| **THE SEAL REWORK v1 (the 4-boss redo)** | 🟡→🔒 · **S0 🔨 branch** | **`BOSS-PLAN.md`** (11 verdicts §V — V#11 ✅) **+ `BOSS-BRIEF.md` (THE BUILD BRIEF, 07-10 — hand-off ready: slices S0–S7 · E1–E9 specs · first-cut numbers · gates)** | `raid_content` (all 4 Seals re-authored) · `combat_core`+`boss_state` (E1 gates+featured · E2 stance · E3 BREAK · E5 mark · E6 deny-empower · E7 listening · E8 kick-window · E9 pips — all guarded) · `encounter_res`+`SealTune` (E4) · `raid_sim` gates · `raid_hud` (castbar lit-slice · stance sigil · pips · break card · mark fuse) · `tune.sh` flags | Fills DESCENT §4 contract 5/7/9/12 **with structure, never +HP**. **S0 sim instrumentation 🔨 BUILT & gate-clean on `seal-rework` (`d8bc675` — byte-identical, det PASS ×4; baseline §BOSSES: all Seals −47…−72% under contract) — NOT merged, holds behind tank-w1 (shared `raid_sim.gd`).** Gates: ① descent map bang **✅ MERGED** (`a59ffa4`/`cf3f8d9`) · ② Wave-1 FLOW=AGGRO/Duelist **in flight (`wow-tank-w1`, at S4)** — **S1+ blocks on its merge** (shared reducer + flow/peel state) · ③ V1–V10 recs = build defaults, one-sentence go opens S2–S5. Order: tank-w1 merges → rebase seal-rework → S1 addenda → S2–S5 fights. Untouched Seals byte-identical per slice. S7 kick re-tune rides the first class `interrupts` flag. ULTRATHINK untouched forever. Worktree `wow-seals`/`seal-rework`. |
...
| THE JAILBREAK interior — SLICE 4 | 🔨 `a22c1ec` (DECK tax ⏳) | DESCENT §7 | `JAILBREAK_LIVE` · `raid_marks` (seat_hp_cut/window_tighten) · `run_director.curses` · `raid_hud` curse core + `_show_jailbreak` + DEPRECATE/Cooling exits · `map_event_panel` `_fx_hint` · `raid_map_sim` walker · new `curse_probe` | **SHIPPED (= TEETH curse-cards).** Curse system (cap 2, pips, ticking, HARD RULE) + two-deal node; bites ECONOMY (mint/price) + HP (temp sector) + TIMING (windows−10% via fresh-per-fight `s.config`); exits Market DEPRECATE (slice-3 slot) + Cooling purge. Byte-identical when dormant. **Deferred:** DECK tax (run-length ability-poison — needs `perform()` gate + spec-threading offline+online) · welded boons (② door) · event-curse legs (③) · online (no-op, NO protocol bump). ⏳ nightly: raid_map_sim run-trace re-baseline. |
