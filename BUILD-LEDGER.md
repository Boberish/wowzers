...
| **THE ANSWER-SHAPE LAW — shape=answer · color=status · size=damage · parry good/bullseye** | 🔨 `tank-icons` (worktree; WIP) | Bill 2026-07-13 (this chat; supersedes the reverted `b1ccf0f` parry-zone + the 16:39 tank-icons-rework assignment) · GRAPHICS-PLAN §2.3.1 | `game/ui/answer_channel.gd` (shape match: heavy→octagon, global→hexagon, beat=neutral diamond; color=status; size scale) · `bands/duelist_band.gd` (HEAVY telegraph beat→octagon kind + size feed; tooltips/hint) · `data/duelist/duelist_kit.gd` (parry graded good/bullseye in `_claim`+`_claim_tg`; octagon=parry-only, bullseye-dodge DELETED) · `duelist_config.gd` (`parry_grade_frac`) · `policies/duelist_policy.gd` (parry-only octagons, parry HEAVY beats) | **◇ dodge-or-parry · ⬡ dodge-only · ⯃ parry-only · ☠ brace.** RED now = peeled (not "boss beat"), blue=flurry, purple=feint. Fixes the invisible-HEAVY-beat (was a plain crimson DODGE diamond, actually parry-only). Guarded/Duelist-only → non-Duelist byte-identical; **`duelist_sim` re-baselines by design** (parry window + heavy legality change). Image-makers build against GRAPHICS-PLAN §2.3.1 (on `main` now). |
...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🔒 **P5/C6A NEXT — reaction-first graybox + theater contract**. P4 DONE: C4 `05d9952` · I2 `11bcd4a`+`567adea` · C5 `f91f1b0` · C5.1 `5bb532c` (Bill: "solid"). C6A retires `hudlow`; **Bill speed/layout gate precedes all I3 generation**, then C6B skins the accepted host. Owed later: scarf 2-piece asset · idle/dump swings · per-boss scenes · retained scene/pose matrix debt · **C6A graybox host 🔨 `2b407c4` — at Bill's rectangle/speed gate; I3/C6B wait** | **`GRAPHICS-PLAN.md` §2.3 · P5 · C6A/C6B** | C6A: `art_v2` dashboard/layout host + selector registration · additive `raid_hud` routing/render null guards · live AnswerChannel/Duelist bindings · party/boss/meter placement · `RaidStage2D` + `SceneKit` shared theater rect · tours/probes. C6B later consumes I3 component assets | **Next owner: Claude C6A. No images.** Default-off legacy remains complete; one responsive contract, no persistent UI in theater, 720/1080/ultrawide + both profiles. Stop after Bill's live rectangle verdict. Any later missing visual source triggers the image-generation stop and must be asked of Bill before Codex generates/edits it. |
