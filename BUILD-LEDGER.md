...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P3 environment pair** (C0/C1 done · C2 merged `da314e9` · **C3 🔨 `1abfcd4`** + **visibility hotfix `516e1b0`** (textures resolve at host _ready, NEVER in painters — WSLg first-load-in-draw = white RID, SCENES.md §3½) · source anchors `3a855ef` · **Stack backdrop+distant 2/5 delivered (`ebd7242`) + RENDERING**, remaining Stack 3 + Cold Aisle 5; **C2+C3 resolution/A-B matrix = retained debt**) | **`GRAPHICS-PLAN.md`** (SUNPRINT CEL + reaction-first verdict locked 07-12; §10 seam map) | LIVE selector + SceneKit six-layer host + `game/art_v2/SCENES.md`; runtime `scenes/stack_atrium/backdrop.png` v1 is the first real layer; legacy/unknown remains `StageBackdrop`; all profiles default-off. Remaining layers must break the base tile's regular symmetry with offset transparent clusters | **Bill V1 approved:** bright, playful, detailed SUNPRINT CEL; UI is the game — dominant answer channel, immediate press/verdict, compact secondary abilities, fast cancelable actor payoff. Old renderer/HUD remains default. P3 proves the bright data-center Stack plus a darker machine-space contrast in the same family. Prototype backdrop is below final source resolution; inspect before upscaling. |
