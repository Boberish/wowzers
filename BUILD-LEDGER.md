...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P4: C4 adapter 🔨 `05d9952` · Codex I2 anchor next** (P3 ✅: Stack Atrium 5/5 + Cold Aisle 5/5; selector/SceneKit/C3 schema live; hotfix `516e1b0`; `PaintedActor2D` + `ACTORS.md` contract live behind the flag — debug slabs on duelist, all other ids fall back to puppets; **retained debt: full matrix + C4 pose/contact tour + live tank playtest**) | **`GRAPHICS-PLAN.md` P4** | Both default-off profiles now contain backdrop/distant/midground/floor/dressing runtime PNGs plus tracked import sidecars; source variants archived outside `res://`; next seam = C4 class-agnostic adapter behind `Actor2D.make()` | **Paired 1080p proof:** all 10 textures render, feet records identical, timing/HUD unchanged. Release-default still forbidden until deferred matrix closes. Next owner: Claude C4 adapter foundation; Codex I2 supplies approved Duelist art; Claude C5 maps it. |
