...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P2 foundation** (C0 done · C1 active · C2 next) | **`GRAPHICS-PLAN.md`** (SUNPRINT CEL + reaction-first verdict locked 07-12; §10 seam map) | future `game/art_v2/` namespace · **exact seams (C0):** `Actor2D.make()` (`actor_2d.gd:58`, + fix the post-purge duelist/alchemist/well→RiftmawRig2D fallthrough) · the ONE `StageBackdrop` at `raid_hud._ready:276` · `_build_combat:2583`+`ClassBand.for_hud` · selector = static `ArtV2` + `--artv2=` in `WorldShell.drive_autostart` | **Bill V1 approved:** bright, playful, detailed SUNPRINT CEL; UI is the game — dominant answer channel, immediate press/verdict, compact secondary abilities, fast cancelable actor payoff. Old renderer/HUD remains default. Mandatory bright-outdoor + darker-interior contrast proof. `stage2d/*`+backdrop = quiet; `raid_hud`/`duelist_band` = live tank surface, merge-main-per-slice. |
