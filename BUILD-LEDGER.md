...
| **Brim — BRANCH SLATE (filing + 4 themes)** | 🟡 | MENDER §9 + `research/brim-sweep.md` | design only (deck reshape = Phase-2 row D5) | Low Catch (Brink-anchored) · Overflow Engine (⭐Reservoir) · Glintsmith (TEAM — Glint ladder + Shining Hour + blindfold) · Deep Well (pulse rhythm). 1 kill (Surgeon), ~7 fixes; distinctness vs Payload/Taproom/Bannerman recorded. |
