...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P3 environment pair** (C0/C1 done · C2 merged `da314e9` · **C3 🔨 `1abfcd4`** + visibility hotfix `516e1b0` · source anchors `3a855ef` · **Stack backdrop+distant `ebd7242` + midground = 3/5 rendering**, remaining floor+dressing + Cold Aisle 5; **C2+C3 resolution/A-B matrix = retained debt**) | **`GRAPHICS-PLAN.md`** (SUNPRINT CEL + reaction-first verdict locked 07-12; §10 seam map) | LIVE selector + SceneKit six-layer host + `game/art_v2/SCENES.md`; runtime Stack backdrop, drifting distant strip, and asymmetric 2048×512 midground are live; legacy/unknown remains `StageBackdrop`; all profiles default-off | **Bill V1 approved:** bright, playful, detailed SUNPRINT CEL; UI is the game — dominant answer channel, immediate press/verdict, compact secondary abilities, fast cancelable actor payoff. 1080p composition keeps timing lane open; judge final density only after floor+dressing. P3 then proves the darker Cold Aisle sibling. |
