...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P3 environment pair** (C0/C1 done · C2 merged `da314e9` · **C3 🔨 `1abfcd4`** — schema/import/bindings live, `stack_atrium`+`stack_cold_aisle` awaiting Codex I1 layers; **C2+C3 resolution/A-B matrix = retained debt**) | **`GRAPHICS-PLAN.md`** (SUNPRINT CEL + reaction-first verdict locked 07-12; §10 seam map) | LIVE `game/art_v2/art_v2.gd` selector + `game/art_v2/scene_kit.gd` six-layer host + `game/art_v2/SCENES.md` (the C3 schema of record: `scenes/<id>/<layer>.png` convention, import defaults, tiling law); legacy/unknown remains `StageBackdrop`; all profiles default-off. Drop Codex layers in the bound folders — zero code per delivery | **Bill V1 approved:** bright, playful, detailed SUNPRINT CEL; UI is the game — dominant answer channel, immediate press/verdict, compact secondary abilities, fast cancelable actor payoff. Old renderer/HUD remains default. P3 proves the bright data-center Stack plus a darker machine-space contrast in the same family. C2 verification debt must close before V2 becomes release default. |
