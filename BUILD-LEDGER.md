...
| **Draw — ABILITY PASS (SKIN + 3 cast transforms, TEMPO-§17.11 treatment)** | 🟡 | **MENDER §13** (2026-07-10) + CARD-CATALOG Draw rows | `data/well/*` (one new cast + per-seat defer buffer in the reducer, guarded) · `well_binds`/`well_gauge` (skin cast + film read) · transform layer on the built release machinery | **SKIN** (the missing-heal film — defers a share of each hit into a ~3s drip; never absorbs/heals; Bill's playtest gap) · **CUPPED HAND** (Flash from the Current, instant/ungraded) · **DEEP DRAW** (Mend's second band, gutter past it) · **THE BRAID** (Cascade as a 3-release string), each a DOOR w/ 2 sub-boons + 1 rig WHEN. Well loaded-count 10→11 — **the 8-cap trim stays PARKED per Bill** (rides the compliance-trims row). |
...
| Item | St | Specced | Touches | Blocks on / note |
|---|---|---|---|---|
| REFIT Phase 4 — SCALE RAILS | 🔨 ~DONE (`b17ff52` `855ac2f` `fcee675` `94b1147` `b4e8d26` `ee58124` + `4779f59` v14) | REFIT §3 P4 | ~~save unification (+roster) · offline `run_seed` · Split-law guard~~ `b17ff52` · ~~vuln stack~~ `855ac2f` · ~~class registry~~ `fcee675` · ~~ClassKit hoists~~ `94b1147` · ~~ClassBand registry~~ `b4e8d26` (`game/ui/bands/`, raid_hud −630) · ~~shared Gauge base~~ `ee58124` (`class_gauge.gd`, 7 widgets) · still open: **TuningConfig literals sweep** (split out) · twinfang kit split | **Wave-0 rails COMPLETE bar two small follow-ups.** Built 2026-07-10 on Bill's go-code. Class reworks now land into band slots + registry rows; net spec-carry unblocked. |
...
| **METER-PLAN — live meter leveled up (L1→L5)** | 🟡 | **METER-PLAN.md** (2026-07-10) | **L1** `meter_panel.gd` + `class_kit.accent()`/`src_label()` hooks + 3 kit overrides (de-stale accent/labels, compact polish) · **L2** `meter_panel.gd` only (⚡AMPLIFY from `boon_meter` · DISCIPLINE from `seat.diag` · sparkline from `series`) · **L3** `run_state`/`run_director` + segment dropdown (= the run-recap row above) · **L4** window chrome + profile store · **L5** teaching layer (`school_of` hook) | **View-only / diag-family → byte gate free** (`ab-gate raid_sim`; `meter_probe` green; `screenshot_meter` WSLg). Data for L1–L2 already exists (STATS PAGE v2). **First slice = L1 + AMPLIFY.** ⚠ L1 fixes a live bug: Alchemist/Well (default seats) have no `_accent`. Duelist accent lands with its kit. At Bill's verdict. |
