...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P4 DONE: C4 `05d9952` · I2 `11bcd4a`+`567adea` · C5 `f91f1b0` · C5.1 registration+animation+grading 🔨 `5bb532c`** (Bill playtested → "solid") — the real Duelist assembled/posed/animating-per-grade default-off; **owed: scarf 2-piece asset · idle/dump swings · per-boss scenes**; next = C6 dashboard (retires the hudlow stopgap); next = P5/C6 dashboard or the P7 side-by-side, Bill's call (P3 ✅ contrast pair; `PaintedActor2D` + `ACTORS.md` live default-off; approved dodge-tank anchor + six alpha parts + two replacement poses archived outside `res://`; debug slabs intentionally remain runtime; **retained debt: full scene matrix + pose/contact tour + live tank playtest**) | **`GRAPHICS-PLAN.md` P4** | C5 consumes `art-source/graphics-v2/p4-duelist/alpha/`: crop/normalize to 300px figure, replace runtime debug PNGs, map pivots/anchors and real pose vocabulary without touching gameplay/legacy factory | **Next owner: Claude C5.** Import/probe + pose/contact tour + live tank playtest; missing-assets fallback and release-default prohibition remain. **No new image generation/substitution:** if art is missing or unusable, Claude stops and asks Bill before Codex generates anything. |
