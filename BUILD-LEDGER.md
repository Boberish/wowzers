...
| **METER-PLAN — live meter leveled up (L1→L5)** | 🔨 `a26a3cd` L1+⚡AMPLIFY+🎯DISCIPLINE+sparklines = **L1–L2 view-only DONE** (2026-07-10) · L3–L5 🟡 | **METER-PLAN.md** (2026-07-10) | ~~**L1** `class_kit.accent()` hook + 5 kit overrides + compact polish~~ `cce7c92` · ~~**L2 ⚡AMPLIFY** (`boon_meter`)~~ `cce7c92` · ~~**L2 🎯DISCIPLINE** (clean-% from `seat.diag`)~~ `88553af` · ~~**L2 sparklines** (per-second trace from `series`)~~ `a26a3cd` · **L2 tail parked (need engine fields):** `src_label()` hook (deferred — `capitalize()` reads fine) · activity% · per-seat interrupt counter · **L3** `run_state`/`run_director` + segment dropdown (= run-recap row above) · **L4** window chrome + profile store · **L5** teaching layer (`school_of` hook) | **L1 + ALL L2 VIEW-ONLY BUILT & MERGED** — `meter_panel.gd` now **6 modes** + row sparklines. All view-only/diag-family; L1+AMPLIFY proved **ab-gate raid_sim BYTE-IDENTICAL**, the rest import clean (byte gate + `screenshot_meter` paused w/ the sim bar `2ee8325`; view-only so byte-safe, **live eyeball owed**). Duelist accent lands with its kit. **Next: L3 segments/run-history** (first slice that touches shared files + wants a real verify). |

### I. THE DESCENT REBUILD (`DESCENT-PLAN.md` — SLICES 1–2 🔨 MERGED `ee18e05` · `1f5e051` 2026-07-10; slices 3–6 next)
