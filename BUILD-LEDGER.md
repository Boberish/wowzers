...
| Item | St | Specced | Touches | Blocks on / note |
|---|---|---|---|---|
| REFIT Phase 4 — SCALE RAILS | 🔨 mostly BUILT (`b17ff52` · `855ac2f` · `fcee675`) | REFIT §3 P4 | ~~save unification (+roster) · offline `run_seed` · Split-law guard~~ **BUILT** `b17ff52` · ~~vuln stack~~ **BUILT** `855ac2f` (§A row) · ~~class registry~~ **BUILT** `fcee675` (`data/class_registry.gd`, `registry_probe`) · still open: ClassBand+Gauge base · ClassKit hoists + TuningConfig sweep · twinfang kit split | **Wave-0 rails.** Bill's go-code 2026-07-10; net integrity hash also shipped same night (`4779f59`, protocol v14). Rails session loop continues: ClassBand+Gauge → hoists. Registry unblocks net spec-carry. |
