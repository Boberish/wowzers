...
| **GRAPHICS DIRECTION V2 — AI-owned modular visual system** | 🟡 **P5 SKIN BUILT: C6B 🔨 `28e9b15`** (I3-B `801d713` bound; C6A `2b407c4`; P4 through C5.1) · **⚠ Bill's live painted-dashboard verdict gates P6/I4/C7** | **`GRAPHICS-PLAN.md` §2.3/§2.3.1 · P5 · C6B (as-built note at the heading)** | `game/art_v2/dash/` (15 pieces + manifest) · `dash_skin.gd` · `dash_party_row.gd` · `dash_host_c6a.gd` · guarded default-off flags in `ui/answer_channel.gd` / `liquid_orb.gd` / `duelist_gauge.gd` / `ability_rune.gd` / `boss_bar.gd` / `boss_cast_bar.gd` · `sim/artv2_dash_prep.gd` / `artv2_dash_states.gd` / `artv2_probe.gd` | Missing-asset ⇒ C6A graybox + legacy chrome (proven). `--artv2=…,dash` only; default-off byte-identical (smoke + ×4 det PASS). After Bill's verdict: approve ⇒ P6 (I4 FX / C7 juice salvage) · revise ⇒ C6B loop. |
