...
**Now:** headless sims per class, UI smokes, screenshot tours, this repo is now GIT (baseline 2026-07-02). Worktree workflow live (see HOW TO WORK). **`scripts/psim.sh <sim> [seeds] [jobs]`** runs a class sim sharded across cores (~4.5× — the sims are single-threaded, fights independent).
...
- ☐ 2026-07-03 · `class-fun-reworks` · §CLASSES — Bill approved the class fun deep-dive leads (memory `class-fun-deepdive`; artifact brief). Per-class Aspect-identity reworks, each own branch, verified & merged one at a time: **① Twinfang Two-Metronomes ✅ (merged `fa3bd36`)** → **② Mender Litany** (ONE inverted-fill combo pip meter [top-early vs catch-late] + Tide-Line/Brink-Band frame reads + Last-Stand/Surge signature fixes) → **③ Bloomweaver Ripen-vs-Snap** (Layered-Garden foundation + ripening grove / snap-streak thorn + Bridge) → **④ Bulwark Chain-vs-Redline + Sunder** (now UNBLOCKED — `bulwark-payexpose-fix` merged `4d3d9b6`; Sunder subsumes payExpose so reconcile with it). Gate each: boonless BYTE-IDENTICAL for untouched classes, determinism PASS, bands retuned to intent, smoke green. *(class-fun session)*
...
- ☐ 2026-07-03 · `raid-sim-parallel` · §TOOLING — wire raid_sim into psim.sh: add byte-identical `--seed0` to raid_sim, make the psim merge header-driven (group before `seed`, won/ttk by name → handles both class + raid schemas), forward extra args (`--boss`). *(this session)*
