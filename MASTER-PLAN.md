...
- ☐ 2026-07-02 · `twinfang-dmg-juice` · §GRAPHICS — Twinfang damage-number juice: tag boss-damage events with their SOURCE (`kind`) in `twinfang_kit._deal` (view-only event field, byte-identical checksums) so the HUD colours **non-auto-attacks distinctly** (Eviscerate/Coup/Rupture/Flurry vs gold Strikes), makes numbers bigger + longer-lived, and gives CRITS bold+outline+scale-punch+spark-ring. Files: `data/twinfang/twinfang_kit.gd` (event tag only), `game/twinfang_hud.gd` (`_dmg_num`). ⚠ shared-risk with `slot-verbs-port` (same two files, different regions — they add rhythm-mod pieces + pips; whoever merges 2nd reconciles). *(this session)*
- ☐ 2026-07-02 · `slot-verbs-port` · §SYSTEMS Phase B port — slot-verb mod pieces for the OTHER FOUR VERBS, same cross-product/no-lockouts pattern, all kit-side + `_b()`-gated: **Twinfang build-your-RHYTHM** (innate proc = PERFECT Strike; opus Twin Step 2nd dodge charge), **Voidcaller build-your-KICK** (innate = landed interrupt; opus Twin Void 2nd kick charge), **Mender build-your-TRIAGE** (innate = clutch heal on a sub-50% ally; opus Benediction every-5th-proc party bathe), **Bloomweaver build-your-GARDEN** (innate = a cashed Bloom; opus Deep Garden payloads ×2 at 3+ Growths). ~8 pieces/class + config knobs + `verb_summary()` + a `_prove_*_mods` probe per class sim; charge pips wire to the twinfang/voidcaller defense runes (LOCK/slot-caption/pip UI is already global). Files: `data/<class>/*_{kit,boons,config}.gd` ×4, `game/{twinfang,voidcaller,mender,bloomweaver}_hud.gd` (tooltip summaries + pips), 4 class sims. Boonless byte-identical bar. ⚠ shared-risk: HUD juice files (`juice2` in flight). *(draft2 session)*
