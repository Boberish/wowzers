...
- ☐ 2026-07-02 · `ability-runes` · §GRAPHICS — **Ability button UI overhaul (Bill, direct: "the ovals are basic — pro")**: rewrite `game/ui/ability_rune.gd` as a chamfered gilded RUNE-SOCKET (obsidian slot + two-tone bevel + big glyph + square radial cd veil + ready gleam sweep + hover ignite/press dip + corner keybind tab + engraved name under the socket). Public API unchanged → no HUD edits; view-only, zero engine files. *(judgment-ui session)*
