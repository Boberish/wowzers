...
- ☐ 2026-07-02 · `juice2` · §GRAPHICS — **Combat juice II (Bill: "QoL + cool animations, make it pro")**: `kill_moment.gd` fight-end beat (SLAIN gold shock / YOU FALL crimson slam, ~1.25s before the end screen; headless-guarded so smokes stay green) wired via an `_on_end` wrapper in all 6 HUDs; BossBar gains the missing **enrage countdown** ("ENRAGE · 8s" pulse → burning ENRAGED frame — the timer was invisible!) + phase-cross gold flash; `_float_num` damage text upgraded (magnitude-scaled Cinzel numerals, drift + arc, late fade) in the 4 HUDs that have it. View-only, zero engine files. *(judgment-ui session)*
