...
- ☐ 2026-07-03 · `fight-seed-map-fix` · §CODE AUDIT — fix confirmed MED bug: `RunState.fight_seed()` collides in Topology map mode (multiple nodes share an enc_index → identical combat RNG → replayed fights). Fix: fold `map_node` in, guarded so `map==null` (linear runs, class sims, draft_sim) stays byte-identical. *(this session)*
