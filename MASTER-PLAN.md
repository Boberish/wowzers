...
| **Draft 2.0 + token economy (Phases A+C)** | ✅ MERGED 2026-07-02 — slot-verbs (Phase B) is the open follow-up (see §SYSTEMS) |
...
- **B. Slot-verbs PoC — OPEN (the next §SYSTEMS task)**: Guard = `[Trigger]+[Property]+[Payload]` mods; rewrite Bulwark's tank boons as typed mods; add the "lock a slot" token spend. **NO LOCKOUTS** — combos stack, rebalance the boss instead. **Scoping rule (locked):** pools stay per-class; mods must express through UI the class already has (new-UI mods get budgeted explicitly or cut). Cross-ASPECT flavor bleed allowed as rare spice only where the class UI already supports it (e.g. Tempo drafting a venom payload — Twinfang UI already renders poison).
...
- ☐ 2026-07-02 · `ui-ceremony` · §GRAPHICS — **Ceremony pass (Bill: "continue the ui updates")**: shared `transition_veil.gd` (obsidian fade on EVERY screen swap — one-line hook in each HUD's `_clear()`), `boss_intro.gd` (Cinzel-Decorative boss title splash + sigil ghost + sweeping gold rules at every fight start, self-freeing overlay), draft-card deal-in stagger (shared DraftScreen — also re-deals on reroll), class-menu card entrance stagger. View-only, zero engine files. ⚠ touches all 6 HUD `_clear`/`_build_combat` + draft_screen + main_menu — staying clear of slot-verbs session (kits/boons). *(judgment-ui session)*
...
- ☐ 2026-07-02 · `slot-verbs` · §SYSTEMS Phase B — build-your-Guard PoC (Bulwark): cross-product mod pieces (TRIGGER proc moments × PAYLOAD effects × PROPERTY verb stats, no lockouts; kit-side proc engine, `slot` field on catalog entries, opus "Twin Guard" 2nd charge) + **LOCK · 1⏣ hold-through-reroll** (`Draft.reroll_kept`, redraws only unlocked slots). Files: `data/bulwark/bulwark_{kit,boons,config}.gd`, `game/draft.gd`, `game/ui/{draft_screen,relic_card,ability_rune}.gd` (charge pips on the NEW rune-socket), `game/bulwark_hud.gd` (spellbook YOUR GUARD + guard tooltip), `sim/{bulwark_sim,draft_sim,screenshot_draft}.gd`. Boonless byte-identical bar as always. ⚠ shared-risk: `ability_rune.gd` (just rebuilt by ability-runes) + bulwark HUD. *(draft2 session)*
