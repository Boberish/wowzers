...
- **MAP-3 (RAID FLOOR 1 — "RING 3: THE SHALLOW STACK") — 3a IN FLIGHT (branch `raid-map`):**
  ...
  **3a scope (this claim): OFFLINE raid map** (human + 3 AI through the floor, per-seat persistent
  integrity, dead raiders reboot at 35% after a won fight, event `draft` fx becomes an emergency
  patch — the raid has no boon draft). **3b (online nav — leader picks the node in the lobby,
  fracs ride the spec) is NOT in 3a**; the map dict is already wire-serializable by design.
...
- ☐ 2026-07-02 · `raid-map` · §MAPS MAP-3a — RAID FLOOR 1 offline: skirmish encounters from the Seal AddRes packs (`data/raid/raid_content.gd`), raid map mode in `game/raid_hud.gd` (per-seat integrity, MapScreen/MapEventPanel reuse), `sim/raid_map_sim.gd`. NOT touching run_state.gd / class HUDs / palette / tuning_config (draft2's surface) or godot/net. *(raid-seals session, continuing)*
