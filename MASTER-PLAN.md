...
- ☐ 2026-07-03 · `sim-parallel` · §TOOLING — speed up the sims: add byte-identical `--seed0` offset to the 5 class sims + a `scripts/psim.sh` parallel sharder (splits seeds across cores, merges CSV, prints bands) — sims are single-threaded on a 12-thread box, each fight independent. *(this session)*
