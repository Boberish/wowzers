...
- ☐ 2026-07-02 · `grimoire` · §GRAPHICS — **Spellbook art pass (continuing Bill's UI-pro sweep)**: shared `game/ui/grimoire.gd` two-page reliquary tome (ABILITIES page: rune-socket icon rows + keybind chips + stat lines + wrapped tips from the ABILITY_TIPS dicts · BOONS page: rarity-gemmed drafted-boon rows from the Draft 2.0 pool dicts, scrollable) replacing the plain-text `_toggle_book` panels in bulwark/twinfang/voidcaller. Healer tome = follow-up. View-only. *(judgment-ui session)*
