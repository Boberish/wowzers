...
- ☐ 2026-07-03 · `bulwark-payexpose-fix` · §CODE AUDIT — fix confirmed MED bug: Bulwark `payExpose` ("Sunder Guard") writes boss-level expose fields only the Voidcaller reads → inert for a solo tank. Fix: per-seat expose window honoring +15% in `outgoing_mult`; stop touching boss state. Boonless byte-identical. *(this session)*
- ☐ 2026-07-03 · `class-fun-reworks` · §CLASSES — Bill approved the class fun deep-dive leads (memory `class-fun-deepdive`; artifact brief). Per-class Aspect-identity reworks, each in its own branch, verified & merged one at a time: **① Twinfang Two-Metronomes** (Tempo accelerando = Flow→BPM · Venom poison-wheel, Flow removed from Venom · +Coup-consumes-Flow / half-Rupture / Flow-milestone procs) → **② Mender Litany** (ONE inverted-fill combo pip meter [top-early vs catch-late] + Tide-Line/Brink-Band frame reads + Last-Stand/Surge signature fixes) → **③ Bloomweaver Ripen-vs-Snap** (Layered-Garden foundation + ripening grove / snap-streak thorn + Bridge) → **④ Bulwark Chain-vs-Redline + Sunder** — BLOCKED until `bulwark-payexpose-fix` merges (heavy BulwarkKit rewrite collides; Sunder subsumes payExpose). Gate each: boonless BYTE-IDENTICAL for untouched classes, determinism PASS, bands retuned to intent, smoke green. *(class-fun session)*
