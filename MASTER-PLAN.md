...
- ☐ 2026-07-03 · `mender-overflow-fix` · §CODE AUDIT — fix the confirmed HIGH bug: Mender `overflow` boon shrinks an already-larger ward + steals ownership (`mender_kit.on_overheal`). Fix: only grow, claim owner on real growth. Boonless byte-identical (gate); overflow-drafted checksums legitimately change (bug fix). *(this session)*
