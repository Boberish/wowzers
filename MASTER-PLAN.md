...
- ☐ 2026-07-02 · `orbs` · §GRAPHICS — **Resource orb overhaul (Bill, direct — after judgment-ui + ability-runes)**: new `ui_orb_liquid.gdshader` (per-pixel liquid: depth gradient, animated surface, meniscus, rising bubbles, damage-chip ghost, gain flash, low-HP boil, fresnel glass + gold rim in ONE pass) + `liquid_orb.gd` rewrite (keeps claw mount/numeral/caption; eased fill, chip trail, flash triggers). API unchanged → all HUD orbs upgrade free. View-only. *(judgment-ui session)*
