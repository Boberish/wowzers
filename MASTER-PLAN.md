...
- ☐ 2026-07-02 · `recap` · §GRAPHICS — **End-screen recap stats (Bill, direct)**: shared `game/ui/recap_panel.gd` ("THE RECKONING") on every win/defeat screen — fight duration, judgment breakdown bar + counts from `seat.diag` (engine truth) in the Judgment Channel's grade colours, damage DEALT/TAKEN stat tiles (view-side event accumulator `RecapPanel.track`), classic parries/kicks/denials, boss-reclaimed-HP, rhythm %, fight epithet (FLAWLESS/CLEAN/SCRAPPY/BLOODY), staggered count-up reveal. Wired into all 6 HUD end screens. View-only, zero engine files. *(judgment-ui session)*
