...
static func make_dense() -> EncounterRes:
	...
	e.melee = {"every": 1.0, "min": 20.0, "max": 28.0, "rhythm": 0.55, "jig": 0.30, "heavy_odds": 0.15}   # §3½: the class's own stream is THE RHYTHM now
	e.enrage_at = 150.0
...
static func make_spike() -> EncounterRes:
	...
	e.melee = {"every": 1.5, "min": 12.0, "max": 18.0, "rhythm": 0.6, "jig": 0.35, "heavy_odds": 0.25}   # §3½: sparser, lazier bars between the spikes
	e.enrage_at = 165.0
	var p0 := PhaseRes.new(); p0.at = 1.0; p0.mult = 1.0; p0.speed = 1.0
