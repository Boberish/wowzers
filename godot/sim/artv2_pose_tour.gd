...
var actors: Array = []
const SHOT_FRAME := 90
const LABELS := ["IDLE", "WINDUP 0.5", "WINDUP 1.0", "WINDUP HEAVY 0.6 (frame)",
	"SWING HEAVY (frame)", "PARRY", "HIT", "DIE"]
...
func _initialize() -> void:
	...
	for i in LABELS.size():
		...
		if act == null:
			...
			quit(1)
			return
		act.position = Vector2(150.0 + float(i) * 225.0, 640.0)
		...
		lb.position = Vector2(80.0 + float(i) * 225.0, 690.0)
...
func _process(_delta: float) -> bool:
	...
	(actors[1] as Actor2D).windup("light", 0.5)
	(actors[2] as Actor2D).windup("light", 1.0)
	(actors[3] as Actor2D).windup("heavy", 0.6)
	if frame == SHOT_FRAME - 45:
		(actors[7] as Actor2D).die()
	if frame == SHOT_FRAME - 6:
		(actors[6] as Actor2D).hit_react(true)
	if frame == SHOT_FRAME - 5:
		(actors[5] as Actor2D).evade_react()
	if frame == SHOT_FRAME - 3:
		(actors[4] as Actor2D).swing("heavy")
	if frame == SHOT_FRAME:
