## screenshot_bloom_raid.gd — visual probe for the SECOND HEALER on the game HUD:
## the class-select (5 cards), the Bloomweaver combat band (Sap orb + Blooming
## Medallion + cast channel + Growth/ward rail), and the meter's new SHIELDING view.
## Needs a display (WSLg — NOT --headless):
##   godot --path godot --script res://sim/screenshot_bloom_raid.gd -- --out=/absolute/dir
extends SceneTree
...
func _initialize() -> void:
	...
	steps = [
		{"name": "bloom_class_select", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._show_class_select(),
			"policy": func(): return null, "ticks": 0, "pre": func(_h): pass},
		{"name": "bloom_raid_wildgrove", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._launch("healer", "wildgrove"),
			"policy": func(): return BloomweaverPolicy.new(),
			"ticks": 420, "pre": func(_h): MeterPanel.view_state = 0},
		{"name": "bloom_raid_thornveil", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._launch("healer", "thornveil"),
			"policy": func(): return BloomweaverPolicy.new(),
			"ticks": 520, "pre": func(_h): MeterPanel.view_state = 0},
		{"name": "bloom_raid_shield_meter", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._launch("healer", "wildgrove"),
			"policy": func(): return BloomweaverPolicy.new(),
			"ticks": 520, "pre": func(h):
				MeterPanel.view_state = 0
				h._meter.mode = "shield"},
	]
...
func _process(_d: float) -> bool:
	match phase:
		0:
			idx += 1
			if idx >= steps.size():
				print("BLOOM RAID TOUR DONE -> ", out_dir)
				return true
			if cur != null:
				cur.queue_free()
				cur = null
