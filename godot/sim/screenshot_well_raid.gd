## screenshot_well_raid.gd — visual probe for THE WELL (reworked direct-cast healer) on
## the game HUD: the WellGauge (charge vessel + Current pips + release band/Still Point +
## verdict banner) for both specs, plus the per-ally Glint chip. Driven by WellPolicy so
## shots catch pours/glints mid-fight.
## Needs a display (WSLg — NOT --headless):
##   godot --path godot --script res://sim/screenshot_well_raid.gd -- --out=/absolute/dir
extends SceneTree
...
func _initialize() -> void:
	...
	steps = [
		{"name": "well_class_select", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._show_class_select(), "ticks": 0},
		{"name": "well_brim_raid", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._launch("healer", "brim"), "ticks": 320},
		{"name": "well_draw_raid", "scene": "res://game/raid_main.tscn",
			"setup": func(h): h._launch("healer", "draw"), "ticks": 320},
	]
...
func _process(_d: float) -> bool:
	match phase:
		0:
			idx += 1
			if idx >= steps.size():
				print("WELL RAID TOUR DONE -> ", out_dir)
				return true
			if cur != null:
				cur.queue_free()
				cur = null
		...
		1:
			...
			pol = WellPolicy.new() if int(st["ticks"]) > 0 else null
			if pol != null:
				pol.latency_ticks = 2
