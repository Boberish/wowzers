...
func _begin_fight() -> void:
	...
	_ctrl.begin(MenderContent.build_fight(_run, run_seed))
	_hover_seat = null
	_focus_seat = null
...
func _show_draft() -> void:
	_screen = "draft"
	_clear()
	var picks := MenderBoons.roll(_run)
	if picks.is_empty():
		_run.enc_index += 1
		_begin_fight()
		return
	...
	_label(head, "The party holds. Take a boon — your Aspect's picks are weighted in.", 15, Palette.TEXT_DIM, HORIZONTAL_ALIGNMENT_CENTER)
	_label(head, _stats_summary(), 13, Palette.STEEL, HORIZONTAL_ALIGNMENT_CENTER)
	var row := HBoxContainer.new()
	row.alignment = BoxContainer.ALIGNMENT_CENTER
