...
func _begin_fight() -> void:
	...
	_ctrl.begin(VoidcallerContent.build_fight(_run, run_seed))
...
func _show_draft() -> void:
	_screen = "draft"
	_clear()
	var picks := VoidcallerBoons.roll(_run)
	if picks.is_empty():
		_run.enc_index += 1
		_begin_fight()
		return
	...
	_title(head, "Attune — take one. Your Aspect's picks are weighted in.", 15, Palette.TEXT_DIM)
...
func _on_card_taken(boon: Dictionary) -> void:
	VoidcallerBoons.apply(boon, _run)
	_run.enc_index += 1
	_begin_fight()
