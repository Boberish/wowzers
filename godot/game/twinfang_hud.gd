...
func _begin_fight() -> void:
	...
	_ctrl.begin(TwinfangContent.build_fight(_run, run_seed))
...
func _show_draft() -> void:
	_screen = "draft"
	_clear()
	var picks := TwinfangBoons.roll(_run)
	if picks.is_empty():
		_run.enc_index += 1
		_begin_fight()
		return
	...
	_title(head, "Sharpen — take one. Your Aspect's picks are weighted in.", 15, Palette.TEXT_DIM)
