...
func _draw() -> void:
	...
	crown.a = 0.10 + 0.05 * sin(pulse * 1.4)
	...
	if dry:
		var dc := Palette.BLOOD.lightened(0.45 + 0.35 * sin(pulse * 4.0))
	...
	if aspect == "draw":
		...
		for i in range(current_max):
			...
			if lit:
				...
				var fl := fmod(pulse * 2.2, float(maxi(current, 1)))
	...
	if t_show:
		...
		if t_band > 0.0:
			...
			var la := 0.85 + (0.15 * sin(pulse * 2.6) if open else 0.0)
			if open:
	...
	if verdict_live():
		var a := clampf(_verdict_t / 0.4, 0.0, 1.0)
		var pop := clampf((1.4 - _verdict_t) * 9.0, 0.0, 1.0)     # fast grow-in
		var bc := Vector2(w * 0.5, -136.0 + 6.0 * (1.0 - pop))
		var f := UiKit.display(800, 2)
		var fs := 21
		var tw := f.get_string_size(verdict, HORIZONTAL_ALIGNMENT_LEFT, -1, fs).x
		UiKit.glow(self, bc, (60.0 + tw * 0.55) * pop,
			Color(verdict_color.r, verdict_color.g, verdict_color.b, 0.34 * a))
		var chip := StyleBoxFlat.new()
		chip.bg_color = Color(Palette.BG0.r, Palette.BG0.g, Palette.BG0.b, 0.90 * a)
		chip.border_color = Color(verdict_color.r, verdict_color.g, verdict_color.b, a)
		...
		UiKit.text_shadowed(self, f, Vector2(cr.position.x, bc.y + 7.0), verdict,
			HORIZONTAL_ALIGNMENT_CENTER, cr.size.x, fs, Color(verdict_color.lerp(Color.WHITE, 0.15), a))
...
func _draw_pool(pool: Rect2, dry: bool) -> void:
	...
	var lw := 70.0
	var lx := pool.position.x + 4.0 + fmod(pulse * 26.0, pool.size.x - lw - 8.0)
...
func _orb(c: Vector2, r: float, fill: float, i: int, newest: bool, dry: bool) -> void:
	...
	if fill >= 0.999:
		...
		var glow_a := 0.22 + 0.06 * sin(pulse * 1.8 + float(i) * 0.7) + 0.30 * (_gain_t if newest else 0.0)
		if newest:
			glow_a += 0.10
		...
		draw_circle(c + Vector2(-r * 0.24, -r * 0.33), r * 0.56,
			water.lerp(Color(0.72, 0.92, 1.0), 0.12 + 0.06 * sin(pulse * 1.8 + float(i) * 0.7)))
		...
		var ph := fmod(pulse * 0.22 + float(i) * 0.43, 1.0)
		...
		if half >= 3.0:
			...
			for k in range(n + 1):
				...
				pts.append(Vector2(c.x - half + 2.0 * half * t,
					lvl + amp * sin(pulse * 2.6 + float(i) * 1.9 + t * 6.0)))
				cols.append(top_c)
			var a0 := asin(dy)
			var a1 := PI - a0
			var m := 10
	...
	if dry:
		ring_base = ring_base.lerp(Palette.BLOOD, 0.35 + 0.30 * sin(pulse * 4.0))
