...
func _charge_dress(x: float, cy: float) -> void:
	# C6C+§11.1 merge: V2 tracks the shared icon geometry (the halo hugs the real
	# 72–88px octagon); legacy keeps the §11.1 readability bump (Bill: "the hold
	# i can't see" — 24 was invisible, 32 reads).
	var r := _size_r("buster", AbilityRes.Size.CRUSH) + 9.0 if v2_skin != null else 32.0
	var c := Vector2(x, cy)
